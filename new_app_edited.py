...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self._transcript: Optional[Transcript] = None
        self._row_widgets: list[dict] = [] # one entry per visible row
    ...
    def _append_row(self, pos: int, etype: str, prob: float, manual: bool):
        ...
        pos_var.trace_add("write", on_change)
        type_var.trace_add("write", on_change)
        prob_var.trace_add("write", on_change)
        ...
        btn = styled_button(self._rows_inner, "✕",
                            lambda ck=1, w=ws: self._delete_row(ck, w),
                            style="danger")
        btn.grid(row=i, column=4, padx=4)
        ws.append(btn)
    ...
    def _rebuild_transcript_from_rows(self):
        if self._transcript is None:
            return None
    ...
    def _write_row(self, *args, **kwargs):
        # Do nothing to transcript here
        # Just mark dirty or trigger render
