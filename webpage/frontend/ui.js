...
function buildParamInputs(params){
    ...
    for(const k in params){
        ...
        const step = spec.step ?? (spec.type === "float" ? 0.01 : 1);
...
function inputHTML(spec, value, rowIndex, key){
    ...
    if(spec.type === "float"){
        let step = spec.step ?? 0.01;
        let min = spec.min ?? 0;
        let max = spec.max ?? 1;

        // For mutually dependent floats
        if(key === "probability"){
            max = 1 - (rows[rowIndex].drop_probability ?? 0);
        } else if(key === "drop_probability"){
            max = 1 - (rows[rowIndex].probability ?? 0);
        }
...
// ----------------- Mutual max for probability/drop -----------------
function updateMutualMax(rowIndex){
    ...
    if(probInput && dropInput && probSlider && dropSlider){
        // Clamp values to ensure sum <= 1
        let prob = parseFloat(probInput.value) || 0;
        let drop = parseFloat(dropInput.value) || 0;

        // Clamp probability
        if(prob + drop > 1){
            if(probInput === document.activeElement){
                prob = 1 - drop; // user is editing probability
            } else {
                prob = Math.min(prob, 1 - drop);
            }
        }

        // Clamp drop_probability
        if(prob + drop > 1){
            if(dropInput === document.activeElement){
                drop = 1 - prob; // user is editing drop
            } else {
                drop = Math.min(drop, 1 - prob);
            }
        }

        // Update number inputs and sliders
        ...
        // Update rows state
        rows[rowIndex].probability = prob;
        rows[rowIndex].drop_probability = drop;

        // Update max for sliders to reflect clamped values
        probInput.max = 1 - drop;
        dropInput.max = 1 - prob;
...
function onFloatInput(rowIndex, key, value){
    let prob = rows[rowIndex].probability;
    let drop = rows[rowIndex].drop_probability;

    if(key === "probability"){
        prob = Math.min(parseFloat(value), 1 - drop);
    } else if(key === "drop_probability"){
        drop = Math.min(parseFloat(value), 1 - prob);
    }
    ...
    // Update number inputs and sliders immediately
    document.getElementById(`p_${rowIndex}_probability`).value = prob;
    document.getElementById(`p_${rowIndex}_drop_probability`).value = drop;
    document.getElementById(`slider_${rowIndex}_probability`).value = prob;
    document.getElementById(`slider_${rowIndex}_drop_probability`).value = drop;
...
// ----------------- Dynamic max for parameter-dependent fields -----------------
